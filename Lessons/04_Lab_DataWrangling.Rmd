...
NTL.phys.data.1990.1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4%in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: we use select for row numbers selection
...
NTL.phys.data.TueLake.1990to1999.July <- 
  NTL.phys.data %>%
...
NTL.phys.data.spread <- pivot_longer(NTL.phys.data, names_to = "depth", values_from= "temperature_C")
